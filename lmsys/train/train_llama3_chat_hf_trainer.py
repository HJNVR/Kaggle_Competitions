...
def main():
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=cfg.batch_size, drop_last=True, collate_fn=custom_collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=cfg.batch_size, shuffle=False, drop_last=True, collate_fn=custom_collate_fn)

    model = LlamaForSequenceClassification.from_pretrained(cfg.model_path, num_labels=cfg.NUM_LABELS, torch_dtype=torch.bfloat16).to(cfg.device)
    ...
    for epoch in range(1, cfg.num_epoch + 1):
        ...
        checkpoint_name = f"/data0/huangjing/workspace/kaggle/lmsys/checkpoint/llama3_ddp/2024_07_10_checkpoint_{epoch:02d}.pth"
